...
export function App(): React.JSX.Element {
  ...
  // ── 小吉集成：暴露 AI 操作接口（ACMS bridge runAction / 动作卡调用）──
  // 每次 render 更新引用（handle 永远指向最新闭包）
  ;(window as any).__sheetsAI = {
    propose: proposeOperations,
    applyPlan: (plan: ChangePlan) => autoApplySafePlan(plan),
    undo: () => {
    ...
    getWorkbookInfo: () => {
      ...
      return {
        fileId: state.file.sessionId,
        name: state.file.name,
        sheets: [...state.file.sheets.map((s: { id: string; name: string }) => ({ id: s.id, name: s.name }))],
      }
    },
  }
