...
/** 浏览器版：内存 pending bytes（shim 注入，替代文件路径队列） */
let pendingBytes: Uint8Array | null = null
/** 浏览器版：注入待打开的内存 pptx 字节（ACMS bridge 调用） */
export function setPendingBytes(bytes: Uint8Array | null): void {
  console.log('[slides-main] setPendingBytes:', bytes ? bytes.length + 'B magic=' + String.fromCharCode(bytes[0], bytes[1]) : 'null')
  pendingBytes = bytes
}
/** 浏览器版：保存回调（shim 上传 /api/office/save）；返回 {ok} 决定保存是否成功 */
...
/** 浏览器版：内存保存（替代 savePptxToFile 写文件） */
async function saveBytesToDisk(opened: OpenedPptx, path: string): Promise<void> {
...
/** 浏览器版：从内存字节打开（替代 openAndBuild 的文件读取） */
...
export function registerSlidesIpc(): void {
  ...
  ipcMain.handle('slides:consume-pending-open', async (e, fitWidthPx: number) => {
    // 浏览器版：优先内存 pending bytes
    if (pendingBytes) {
      const bytes = pendingBytes
      pendingBytes = null
      console.log('[slides-main] consume pending bytes:', bytes.length, 'B, magic:', String.fromCharCode(bytes[0], bytes[1]))
      try {
        return await openBytes(e.sender, bytes, 'local.pptx', fitWidthPx)
      } catch (err: any) {
        console.error('[slides-main] openBytes 失败:', err.message)
        return null
      }
    }
  ...
  ipcMain.handle('slides:save', async (e) => {
    ...
    try {
      await saveBytesToDisk(session.opened, session.path)
  ...
  ipcMain.handle('slides:save-as', async (e, defaultName: string) => {
    ...
    try {
      await saveBytesToDisk(session.opened, r.filePath)
