...
### 问题 1: `_streamSpeed` 未定义

**原因**: 变量在 `sendMessage` 函数内部使用，但未在模块作用域声明。
...
### 问题 2: `window.buddyStreamPause` 等函数未定义

**原因**: 函数在 `sendMessage` 内部定义，但面板按钮在初始化时就尝试绑定事件。

**修复**: 将函数定义移到 `ensurePanel` 中，确保按钮事件绑定时函数已存在：
```javascript
// 在 ensurePanel 中
