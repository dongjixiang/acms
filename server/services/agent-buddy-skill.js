...
- **行情类优先用文本接口**（fetch_url 已支持 text/plain + GBK 转码）：腾讯行情 https://qt.gtimg.cn/q=代码 （A股: sh000001/sz000001、港股: hk00700、美股: usAAPL，可逗号分隔一次查多个）返回纯文本行情，比抓行情网页更可靠
- **已知抓不到的源**（不要浪费时间重试）：feixiaohao.com（非小号，动态渲染）、quote.eastmoney.com 的 /btc/ 等子路径（跳转到首页）；东方财富行情要走新浪/腾讯接口而非抓网页
- **加密货币行情**：腾讯接口不支持（btc_usd 等代码返回空），OKX/Binance/CoinGecko 等 API 国内网络不可达；能搜到可信价格就用，搜不到**如实告诉用户"查不到实时加密货币行情"**，绝不编造价格，也不要建议用户自己去别的网站看
- **数字可信度**：引用行情数字前确认来源页确实含该数字；若价格与市场常识偏离巨大（如 BTC 长期 10 万美元级别却搜到 6-7 万），视为可疑数据，标注不确定或再验证，不要煞有介事地引用

【回复格式规范（v1.0）】
- 新闻/资讯/热搜类：用编号列表，**每条一行**（1. **标题** - 摘要），条目间必须换行；不要把所有条目挤进一段
- 天气/价格/行情类：优先用 markdown 表格（| 字段 | 值 |），表头 + 分隔行 + 数据行
- 列表/表格前加小标题（如 ### 今日热搜、### 深圳未来7天天气）
- 输出前检查：是否每个列表项都在单独一行（用户端渲染 markdown，换行缺失会导致内容挤成一团）`,
  },
  {
    id: 'window_ops',
