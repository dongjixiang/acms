...
// v0.63: Planner 自我报告"无法理解"检测模式

//   任务描述空泛时（"test"/4字符），Planner 探索 N 轮后可能主动说"无法理解"

//   匹配这些 pattern → abort multi_role + 写 progress_note 告诉 PM 补充 description

//   注意：只匹配"无法理解任务"的明确表达，不误判"无法找到文件 X"等合法 plan 内容

const PLANNER_CANNOT_UNDERSTAND_PATTERNS = [
 ...
 * 检测 Planner 输出是否表明 LLM 无法理解任务

 * @returns {{understood: boolean, reason: string, reasonLabel: string}}

 */

function checkPlannerUnderstanding(runtimeResult) {

  const content = (runtimeResult.content || '').trim();



  // 1. 输出过短：< 50 字符几乎肯定不是 plan（plan 至少含 files/steps 描述）
  ...
  if (content.length < 50) {
    ...
    return {
      ...
      reason: 'Planner output too short (<50 chars) — task likely too vague to plan',

      reasonLabel: 'Planner 输出过短（<50 字符），可能无法形成规划',
  ...
  // 2. 关键词匹配：LLM 主动表达"无法理解"

  for (const pattern of PLANNER_CANNOT_UNDERSTAND_PATTERNS) {
    ...
    if (pattern.test(content)) {
      ...
      return {
        ...
        reason: 'Planner self-reports cannot understand task (matched explicit pattern)',

        reasonLabel: 'Planner 明确表示无法理解任务',
...
async function runMultiRoleSequence(task, options = {}) {
  ...
  for (let idx = 0; idx < pipeline.length; idx++) {
    ...
    if (idx === 0) {
      ...
      // v0.63 E: 非首个角色（Coder/Tester/Reviewer）注入前序角色输出
      //   - Coder 阶段（idx===1）：优先从 task.doc.plan.content 读（Planner 已写入，
      //     不截断 1500 字）。fallback 到 roleOutputs.planner.content（兼容旧数据）
      //   - Tester/Reviewer 阶段（idx>=2）：保持原逻辑从 roleOutputs 注入
      if (idx === 1 && task.plan?.content) {
        const planContent = task.plan.content;
        const planMeta = task.plan.generatedBy
          ? `(generated by ${task.plan.generatedBy}, auto-approved)`
          : '';
        msgs.push({
          role: 'user',
          content: `## Planner 已批准的 plan${planMeta ? ' ' + planMeta : ''}\n\n${planContent}\n\n请基于以上 plan 直接进入实现阶段，不要重新 explore workspace。`,
        });
      } else {
...
// 执行
    ...
    // v0.63: Planner 阶段专属检测 — LLM 是否能理解任务
    //   任务描述空泛时（"test"/4字符），Planner 探索 N 轮后可能主动说"无法理解"，
    //   此时继续走 Coder/Tester/Reviewer 只会让 LLM 瞎猜，浪费 token。
    //   主动 abort + 写 progress_note 告诉 PM 补充 description。
    if (phase.name === 'planner') {
      const verdict = checkPlannerUnderstanding(runtimeResult);
      if (!verdict.understood) {
        const errorMsg = `[${phase.label}] ${verdict.reasonLabel}`;
        console.warn(`[task-agent] ${errorMsg}`);
        console.warn(`[task-agent] Planner 输出前 300 字符: "${(runtimeResult.content || '').slice(0, 300).replace(/\n/g, ' | ')}"`);
        saveProgress(errorMsg);
        // 写 progress_note 让 PM 在 UI 看到具体原因
        const llmContentPreview = (runtimeResult.content || '(空)').slice(0, 800);
        const progressNote = `⚠️ Planner 在 ${phase.maxRounds} 轮探索后无法理解任务。\n\n`
          + `**根因**：${verdict.reason}\n\n`
          + `**可能原因**：\n`
          + `- description 太短（建议 ≥20 字符，给 LLM 足够上下文）\n`
        ...
        try {
          await eventBus.emit('task.failed', {
            ...
            payload: {
              ...
              rawDiag: runtimeResult.raw || null,
              llmContentPreview: llmContentPreview.slice(0, 500),
              progressNote,
            },
          });
      } catch (e) { /* 不阻塞 */ }
        // v0.63 #2: abort 时 transition 到 failed（让 PM 在 UI 立刻看到失败状态）
        //   之前 task.status 还在 in_progress，ai-tools.js 路由层会误走 audit+submit
        try {
          taskStore.update(taskId, {
            status: 'failed',
            progress: 0,
        ...
        return {
          ...
          modelUsed: model.id,
          analysis: errorMsg,
          // v0.63 #1: aborted 标记，让 ai-tools.js:274 路由层不再走 audit+submit
          aborted: true,
          abortedRole: phase.name,
          failureReason: verdict.reason,
          completedAt: new Date().toISOString(),
          roles: { [phase.name]: { ok: false, length: (runtimeResult.content || '').length, understood: false, reason: verdict.reason } },
        };
      }
    }
    ...
    // v0.63 E: Planner 完成后把 plan 持久化到 task.doc.plan（结构化衔接）
    //   之前 Planner 输出只在内存 roleOutputs 中，被 .slice(0, 1500) 截断后传给 Coder
    //   现在显式写 task.doc.plan（不截断），Coder 直接读 plan.content
    //   顺带：PM 在前端能看到 plan，multi_role 自动 approved（不需要走 /agent-plan-approve 审批流）
    if (phase.name === 'planner') {
      try {
        const planContent = (runtimeResult.content || '').trim();
        const autoPlan = {
          content: planContent,
          model: model.id,
          ...
          generatedBy: 'multi_role_planner',
        };
        taskStore.update(taskId, {
          plan: autoPlan,
          plan_status: 'auto_approved',
        });
        saveProgress(`[${phase.label}] plan 已写入 task.doc.plan (${planContent.length} chars)`);
      } catch (e) {
        console.warn(`[task-agent] 写 task.doc.plan 失败: ${e.message}`);
      }
    }
...
//   v0.88: 已抽出到 server/tools/delegate-subtasks.js 独立注册（任何入口可见）
// ═══════════════════════════════════════════════════════════
